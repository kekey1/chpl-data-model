...
then
	...
	pg_dump --host $HOST --username $USER --port $PORT --no-password --format custom --blobs --verbose --exclude-table-data=quartz.* --exclude-table-data=ff4j.* --exclude-table-data=audit.logged_actions --exclude-table-data=openchpl.api_key_activity --file $FILE openchpl
fi
